...
We see that the CRS (`coord. ref.`) is in NAD83. Since this matches the CRS of our vector data we can carry on with analysis without re-projecting. However, if you did want to transform a raster object to a different CRS you would use the `project()` function from the `terra` package.
...
Then, make a barplot that compares the elevation at each of the 3 sites. *Hint : use `geom_col()`*. Which site has the highest elevation? (7 pts).
