...
There are a few different functions to conduct a PCA in R (many available in additional R packages such as the `vegan` package for community analyses), but we are going to use the `prcomp()` function from base R, a reliable and stable function for conducting PCA.
...
Looks like the first 2 axes explain nearly all the variance of the data space (\>80%, which is often the desired outcome).
...
We can view more patterns in the data by coloring points (i.e., seeding samples) by one of the other variables in our data set. This project involved assessing the impacts of Calcium addition on sugar maple seedlings by comparing seedlings traits and calcium treated (W1) and untreated (Reference) sites. Therefore let's color points by watershed treatment (the `watershed` variable) to see if there is any clustering in seedlings traits among treatments.

To do so with the `autoplot()` function we also specify the original dataset we used in `prcomp()` and the name of the column we want to color by. *(Note the `colour:` argument is spelled the British way.)*

```{r}
autoplot(maples03_pca, data = maples03, colour = "watershed")

```

This plot definitely shows some clustering among sites along the first PC axis.

We can make ggplot objects interactive with the `plotly` package by feeding it into the `ggplotly()` function.
...
Report/interpret some of the results. Looking at biplot, interpret variable loadings based on vector angles (lecture material).

Make a biplot coloring samples by elevation. Do you notice any clustering?

Add PC1 and PC2 as columns in maples03 and perform a t-test between watershed treatment using the PC1 axis as the response. Is there a significant difference in plant traits among treatments?
